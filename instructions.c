...
void inst_to_str(cpu_context* ctx, char* str) {
    ...
    sprintf_s(str, "%s ", inst_name(inst->type));
    ...
    case AM_R_D16:
    case AM_R_A16:
        sprintf_s(str, "%s %s,$%04X", inst_name(inst->type),
            rt_lookup[inst->reg_1], ctx->fetched_data);
        return;

    case AM_R:
        sprintf_s(str, "%s %s", inst_name(inst->type),
            rt_lookup[inst->reg_1]);
        return;

    case AM_R_R:
        sprintf_s(str, "%s %s,%s", inst_name(inst->type),
    ...
    case AM_R_D8:
    case AM_R_A8:
        sprintf_s(str, "%s %s,$%02X", inst_name(inst->type),
    ...
    case AM_A8_R:
        sprintf_s(str, "%s $%02X,%s", inst_name(inst->type),
            bus_read(ctx->regs.pc - 1), rt_lookup[inst->reg_2]);

        return;

    case AM_HL_SPR:
        sprintf_s(str, "%s (%s),SP+%d", inst_name(inst->type),
            rt_lookup[inst->reg_1], ctx->fetched_data & 0xFF);
        return;

    case AM_D8:
        sprintf_s(str, "%s $%02X", inst_name(inst->type),
            ctx->fetched_data & 0xFF);
        return;

    case AM_D16:
        sprintf_s(str, "%s $%04X", inst_name(inst->type),
            ctx->fetched_data);
        return;
    ...
    case AM_A16_R:
        sprintf_s(str, "%s ($%04X),%s", inst_name(inst->type),
